...
class FittingMonitor(object):
    def __init__(self, summary_steps=1, visualize=False,
                 maxiters=1000, ftol=2e-09, gtol=1e-05,
    ...
    def run_fitting(self, optimizer,closure, params, body_model,
                    use_vposer=True, pose_embedding=None, vposer=None,scheduler=None,
                    **kwargs):
    ...
    def create_fitting_closure(self,
                               ...
                               gt_joints=None,visibility=None, joints_smooth=None, idx=0, loss=None,
                               joints_conf=None,
                               joint_weights=None,
                               ...
                               camera_orient=None,
                               betas_fix=None,
                               return_fix=False,scheduler=None,
                               **kwargs):
...
class SMPLifyLoss(nn.Module):
    ...
    def forward(self, body_model_output, camera, gt_joints, visibility, joints_smooth, idx, joints_conf,
                body_model_faces, joint_weights,
                use_vposer=False, pose_embedding=None,
                **kwargs):

        projected_joints = camera(body_model_output.joints)
        joints_to_smooth = body_model_output.body_pose.reshape(1,21,3)
        ...
        upstanding_depth_loss = torch.sum(self.robustifier(body_model_output.joints[:, 1, 2] - body_model_output.joints[:, 8:11, 2]))+ \
                                torch.sum(self.robustifier(body_model_output.joints[:, 1, 2] - body_model_output.joints[:, 12:14, 2]))
