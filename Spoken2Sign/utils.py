...
def init_DDP():
    ...
    torch.cuda.set_device(local_rank % num_gpu)
    device = torch.device('cuda:{}'.format(local_rank % num_gpu))
    torch.distributed.init_process_group(backend="nccl", init_method="env://", timeout=datetime.timedelta(0,3600*48))
