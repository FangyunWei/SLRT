...
class RecognitionNetwork(torch.nn.Module):
    def __init__(self, cfg, input_type, transform_cfg, 
        ...
        if self.input_type=='video':
            ...
            elif 'keypoint' in input_streams and not 'rgb' in input_streams and input_streams != ['keypoint', 'keypoint']:
            ...
            elif ('rgb' in input_streams and 'keypoint' in input_streams) or input_streams == ['keypoint', 'keypoint']:
                self.visual_backbone_twostream = S3D_two_stream_v2(
                    rgb_input_channels=cfg['s3d'].get('in_channel', 3),
                    use_block=cfg['s3d']['use_block'],
                    freeze_block=(cfg['s3d']['freeze_block'], cfg['keypoint_s3d']['freeze_block']),
                    pose_inchannels=cfg['keypoint_s3d']['in_channel'],
                    flag_lateral=(cfg['lateral'].get('pose2rgb',False),
        ...
        if 'visual_head' in cfg:
            ...
            if 'rgb' in input_streams or input_streams == ['keypoint', 'keypoint']:
            ...
            if 'triplehead' in self.fuse_method:
                assert ('rgb' in input_streams and 'keypoint' in input_streams) or input_streams==['keypoint','keypoint']
        ...
        if 'pretrained_path_rgb' in cfg:
            ...
            if self.visual_backbone!=None and self.visual_backbone_twostream==None:
                neq_load_customized(self.visual_backbone, backbone_dict, verbose=(os.environ['LOCAL_RANK']=='0'))
                neq_load_customized(self.visual_head, head_dict, verbose=(os.environ['LOCAL_RANK']=='0'))
                ...
                neq_load_customized(self.visual_backbone_twostream.rgb_stream, backbone_dict, verbose=(os.environ['LOCAL_RANK']=='0'))
                neq_load_customized(self.visual_head, head_dict, verbose=(os.environ['LOCAL_RANK']=='0'))
                if cfg['pyramid']['rgb'] == 'shared_head' and fc_dict!={}:
                    neq_load_customized(self.fc_layers_rgb, fc_dict, verbose=(os.environ['LOCAL_RANK']=='0'))
                elif cfg['pyramid']['rgb'] == 'multi_head' and head_remain_dict!={}:
                    neq_load_customized(self.visual_head_remain, head_remain_dict, verbose=(os.environ['LOCAL_RANK']=='0'))
        ...
        if 'pretrained_path_keypoint' in cfg:
            ...
            if self.visual_backbone_keypoint!=None and self.visual_backbone_twostream==None:
                neq_load_customized(self.visual_backbone_keypoint, backbone_dict, verbose=(os.environ['LOCAL_RANK']=='0'))
                neq_load_customized(self.visual_head_keypoint, head_dict, verbose=(os.environ['LOCAL_RANK']=='0'))
                ...
                neq_load_customized(self.visual_backbone_twostream.pose_stream, backbone_dict, verbose=(os.environ['LOCAL_RANK']=='0'))
                neq_load_customized(self.visual_head_keypoint, head_dict, verbose=(os.environ['LOCAL_RANK']=='0'))
                if cfg['pyramid']['pose'] == 'shared_head':
                    neq_load_customized(self.fc_layers_keypoint, fc_dict, verbose=(os.environ['LOCAL_RANK']=='0'))
                elif cfg['pyramid']['pose'] == 'multi_head':
                    neq_load_customized(self.visual_head_keypoint_remain, head_remain_dict, verbose=(os.environ['LOCAL_RANK']=='0'))
    ...
    def augment_two_heatmaps(self, is_train, datasetname, sgn_heatmaps=None, sgn_heatmaps_extra=None):
        current_transform_cfg = self.transform_cfg[datasetname]
        hm_h, hm_w = self.heatmap_cfg[datasetname]['input_size'], self.heatmap_cfg[datasetname]['input_size']
        if is_train:
            i2, j2, h2, w2 = torchvision.transforms.RandomResizedCrop.get_params(
            ...
            sgn_heatmaps = self.apply_spatial_ops(
                    ...
                    spatial_ops_func=lambda x:torchvision.transforms.functional.resized_crop(
                        x, i2, j2, h2, w2, [hm_h, hm_w]))
            sgn_heatmaps_extra = self.apply_spatial_ops(
                    sgn_heatmaps_extra,
                    spatial_ops_func=lambda x:torchvision.transforms.functional.resized_crop(
                        x, i2, j2, h2, w2, [hm_h, hm_w]))
        else:
            ...
            spatial_ops = torchvision.transforms.Compose(spatial_ops)
            sgn_heatmaps = self.apply_spatial_ops(sgn_heatmaps, spatial_ops)
            sgn_heatmaps_extra = self.apply_spatial_ops(sgn_heatmaps_extra, spatial_ops)  

        sgn_heatmaps = (sgn_heatmaps-0.5)/0.5
        sgn_heatmaps = sgn_heatmaps.permute(0,2,1,3,4).float()
        sgn_heatmaps_extra = (sgn_heatmaps_extra-0.5)/0.5
        sgn_heatmaps_extra = sgn_heatmaps_extra.permute(0,2,1,3,4).float()
        return sgn_heatmaps, sgn_heatmaps_extra
        ...
        if self.input_type=='video':
            ...
            with torch.no_grad():
                ...
                if 'keypoint' in self.input_streams:
                    ...
                    if self.input_streams == ['keypoint', 'keypoint']:
                        sgn_heatmaps = self.generate_batch_heatmap(
                            sgn_keypoints[0], datasetname) #B,T,N,H,W
                        sgn_videos = self.generate_batch_heatmap(
                            sgn_keypoints[1], datasetname) #B,T,N,H,W
                    else:
                        sgn_heatmaps = self.generate_batch_heatmap(
                ...
                if self.input_streams != ['keypoint', 'keypoint']:
                    ...
                    sgn_heatmaps, sgn_videos = self.augment_two_heatmaps(
                        is_train=is_train, datasetname=datasetname, 
                        sgn_heatmaps=sgn_heatmaps, sgn_heatmaps_extra=sgn_videos)
            ...
            elif 'keypoint' in self.input_streams and not 'rgb' in self.input_streams and self.input_streams != ['keypoint', 'keypoint']:
            ...
            elif ('rgb' in self.input_streams and 'keypoint' in self.input_streams) or self.input_streams == ['keypoint', 'keypoint']:
                # if name[0]=='pseudo':
            ...
            if self.fuse_method=='empty':
                ...
                assert ('rgb' in self.input_streams and 'keypoint' in self.input_streams) or self.input_streams == ['keypoint','keypoint']
