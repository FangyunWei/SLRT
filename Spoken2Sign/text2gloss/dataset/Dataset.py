...
from utils.keypoints_3d_aug import create_smplx_model
...
Hrnet_Part2index = {
    ...
    'face_no_contour': [('keypoints', list(range(40, 71)))]
}
for k_ in ['mouth','face_others', 'face_no_contour', 'hand', 'pose']:
...
class SignLanguageDataset(torch.utils.data.Dataset):
    def __init__(self, dataset_cfg, split):
        ...
        if 'keypoint_file' not in self.dataset_cfg:
            self.dataset_cfg['keypoint_file'] = None
        if type(self.dataset_cfg['keypoint_file']) == list:
            self.name2keypoints = self.load_keypoints(self.dataset_cfg['keypoint_file'][0])
            self.name2keypoints_extra = self.load_keypoints(self.dataset_cfg['keypoint_file'][1])
        else:
            self.name2keypoints = self.load_keypoints(self.dataset_cfg['keypoint_file'])
            self.name2keypoints_extra = None
    ...
    def load_keypoints(self, keypoint_file=None):
        name2keypoints = None
        if 'keypoint' in self.input_streams and keypoint_file is not None:
            ...
            for name, all_keypoints in name2all_keypoints.items():
                ...
                for k in sorted(self.dataset_cfg['use_keypoints']):
                    for key_, selected_index in Part2index[k]:
                        try:
                            name2keypoints[name].append(all_keypoints[key_][:,selected_index]) # T, N, 3
                        except: 
                            name2keypoints[name].append(all_keypoints[:,selected_index]) # T, N, 3
                        kp_selected_indexs.extend(selected_index)
                name2keypoints[name] = torch.tensor(np.concatenate(name2keypoints[name], axis=1)) #T, N, 3
                self.keypoints_num = name2keypoints[name].shape[1]
                if len(name2keypoints)==1:
            ...
            assert self.keypoints_num==len(kp_selected_indexs)
            self.kp_selected_indexs = kp_selected_indexs
