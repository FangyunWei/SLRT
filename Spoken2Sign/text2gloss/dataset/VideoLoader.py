...
from utils.keypoints_3d_aug import kps_aug
...
def get_selected_indexs(vlen, tmin=1, tmax=1, num_tokens=1, max_num_frames=300):
...
def read_img(path, dataset_name, csl_cut, csl_resize=-1):
    ...
    if '.zip' in path:
        ...
        try:
            ...
            # print('broken img: ', path)
            rgb_im = Image.fromarray(np.zeros(shape+[3], dtype=np.uint8))
...
def load_batch_video(zip_file, names, num_frames, transform_cfg, dataset_name, is_train, 
        ...
        pseudo_cfg={},max_num_frames=400, need_video=True, name2keypoint_extra=None):
    #load_video and then pad
    if name2keypoint!=None:
        assert pad=='replicate', 'only support pad=replicate mode when input keypoints'
    sgn_videos, sgn_keypoints, sgn_keypoints_extra = [], [], [] # B,C T,H,W
    sgn_lengths = [] # B
    sgn_selected_indexs = []
    ...
    for ii, (name, num) in enumerate(zip(names, num_frames)):
        ...
        video, len_, selected_indexs, pseudo_outputs_ = load_video(
            ...
            is_pseudo=('pseudo' in name), is_train=is_train, max_num_frames=max_num_frames, need_video=need_video)
        sgn_lengths.append(len_)
        sgn_videos.append(video) #wo transformed!!
        if name2keypoint != None:
            kps = name2keypoint[name][selected_indexs,:,:]
            sgn_keypoints.append(kps) # T, N, D
            if name2keypoint_extra is not None:
                sgn_keypoints_extra.append(name2keypoint_extra[name][selected_indexs,:,:])
            else:
                sgn_keypoints_extra.append(None)
        else:
    ...
    padded_sgn_videos, padded_sgn_keypoints, padded_sgn_keypoints_extra = [], [], []

    for video, keypoints, keypoints_extra, len_ in zip(sgn_videos, sgn_keypoints, sgn_keypoints_extra, sgn_lengths):
        if need_video:
            video = pil_list_to_tensor(video, int2float=True) #T,C,H,W
        if len_<max_length and need_video:
            if pad=='zero':
        ...
        if name2keypoint!=None:
            ...
            if len_<max_length:
                ...
                if name2keypoint_extra is not None:
                    padded_sgn_keypoints_extra.append(keypoints_extra)
    ...
    if name2keypoint!=None:
        ...
        if name2keypoint_extra is not None:
            sgn_keypoints = [sgn_keypoints, torch.stack(padded_sgn_keypoints_extra, dim=0)]
    else:
        sgn_keypoints = None
...
datasetname2format = {
    ...
    'phoenix_syn': '{}/images_fix_connector/{}/images{:04d}.png',
    'phoenix_syn_gt': '{}/images_mesh/{}', #'{}/images/{}/images{:04d}.png',
    'csl_syn_gt': '{}/images_mesh/{}',
    'phoenix_syn_smplx': '{}/images_smplx/{}/images{:04d}.png',
    'phoenix_syn_smplx_gt': '{}/images_smplx_gt/{}/images{:04d}.png',
}
def load_video(zip_file, name, num_frames, name_sequence, boundary_sequence, transform_cfg, dataset_name, is_pseudo, is_train, max_num_frames, need_video):
    ...
    if 'temporal_augmentation' in transform_cfg and is_train:
        ...
        tmin, tmax = 1, 1
    if dataset_name.lower() in ['csl', 'phoenix', 'phoenix2014tsi', 'phoenix2014', 'phoenix2014si', 'tvb', 
                                'phoenixcomb', 'phoenix_syn', 'phoenix_syn_gt', 'csl_syn_gt', 'phoenix_syn_smplx', 'phoenix_syn_smplx_gt']: #read 2D image
        ...
        for name, (start, end) in  zip(name_sequence, boundary_sequence):
            ...
            if name.endswith('_3d_aug'):
                name = name[:-7]
            image_path_list.extend([path_format.format(zip_file, name, fi) 
                for fi in range(start, end+1)])#end inclusive
        if 'phoenix_syn' in dataset_name.lower() or dataset_name.lower() in ['csl_syn_gt']:
            image_path_list = sorted(os.listdir(path_format.format(zip_file, name)))
            image_path_list = [os.path.join(path_format.format(zip_file, name), img_path) for img_path in image_path_list]
            selected_indexs, valid_len = get_selected_indexs(num_frames, tmin=tmin, tmax=tmax, max_num_frames=max_num_frames)
        ...
        #     raise ValueError  
            ...
            selected_indexs, valid_len = get_selected_indexs(len(image_path_list), tmin=tmin, tmax=tmax, max_num_frames=max_num_frames)
