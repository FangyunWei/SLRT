...
def fit_single_frame(img,idx,
    ...
    with fitting.FittingMonitor(
        ...
        camera_optimizer, camera_create_graph = optim_factory.create_optimizer(
            ...
            **kwargs)
        milestones = [1, 2]
        scheduler_camera = torch.optim.lr_scheduler.MultiStepLR(camera_optimizer, milestones, gamma=1e1, last_epoch=-1,
                                                         verbose=False)
        ...
        fit_camera = monitor.create_fitting_closure(
            ...
            use_vposer=use_vposer, vposer=vposer,
            pose_embedding=pose_embedding,
            return_full_pose=False, return_verts=False,scheduler=scheduler_camera)
        ...
        cam_init_loss_val = monitor.run_fitting(camera_optimizer,
                                                ...
                                                pose_embedding=pose_embedding,
                                                vposer=vposer,scheduler=scheduler_camera)
  ...
  #              cam_init_loss_val))
        ...
        for or_idx, orient in enumerate(orientations):
            ...
            for opt_idx, curr_weights in enumerate(opt_weights):
                ...
                scheduler = torch.optim.lr_scheduler.MultiStepLR(body_optimizer,milestones,gamma=1e1,last_epoch=-1,
                                                                 verbose=False)
                ...
                closure = monitor.create_fitting_closure(
                    ...
                    use_vposer=use_vposer, vposer=vposer,
                    pose_embedding=pose_embedding,
                    return_verts=True, return_full_pose=True,scheduler=scheduler)
                ...
                final_loss_val = monitor.run_fitting(
                    ...
                    use_vposer=use_vposer,scheduler=scheduler)

                if interactive:
                    if use_cuda and torch.cuda.is_available():
                        torch.cuda.synchronize()
                    elapsed = time.time() - stage_start
                    # if interactive and is_main_process():
    ...
    if final_loss_val is None:
        nan_flag = True
    return joints_smooth, camera_transl, camera_orient, betas_fix, nan_flag, results
