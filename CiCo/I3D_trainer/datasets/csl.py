...
class CSL(VideoDataset):
    def __init__(
        ...
        self.json_info=load_dict('misc/csl/csl_train_test_info.json')
        self.train = list(np.where(np.asarray(self.json_info["split"]) == 'train')[0])
        self.valid = list(np.where(np.asarray(self.json_info["split"]) == 'test')[0])
