...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(self, trg, enc_src, src_mask,attn_mask=None):
        ...
        x = self.ln_3(x)
        # x = trg + self.attention(self.ln_1(trg), enc_src, src_mask)
        # x = self.ln_2( x)
        x=x.permute((1, 0, 2))
...
class FeatureTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,feature_len: int,
                 linear_patch: str = '2d',):
        super().__init__()
        self.input_resolution = input_resolution
        self.output_dim = output_dim
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        # self.mlp = nn.Sequential(OrderedDict([
        #
        #     ("c_fc", nn.Linear(1024*8, 1024*2)),
        #     ("gelu", QuickGELU()),
        #     ("c_proj", nn.Linear(1024*2, 512))
        #
        # ]))
